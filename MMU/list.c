...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  curr = l->head;
  prev = l->head;
  int blocksize = temp->blk->end - temp->blk->start;
  int curr_blocksize = curr->blk->end - curr->blk->start + 1;

  if (!curr->next) {
    if (blocksize <= curr_blocksize) {
      temp->next = l->head;
      l->head = temp;
    } else {
      temp->next = NULL;
    ...
    if (blocksize <= curr_blocksize) {
      ...
      while(curr && blocksize > curr_blocksize) {
        prev = curr;
        curr = curr->next;
        if (curr) {
          curr_blocksize = curr->blk->end - curr->blk->start;
        }
      }
      prev->next = temp;
      temp->next = curr;
